...
const number = pz
  .terminal(JsonToken.Number, "<number>")
  .map((t) => Number(t.text))
  .followedBy(whitespace);
const string = pz
  .terminal(JsonToken.String, "<string>")
  .map((t) => JSON.parse(t.text) as string)
  .followedBy(whitespace);
const littrue = pz
  .terminal(JsonToken.True, "true")
  .map(() => true)
  .followedBy(whitespace);
const litfalse = pz
  .terminal(JsonToken.False, "false")
  .map(() => false)
  .followedBy(whitespace);
const litnull = pz
  .terminal(JsonToken.Null, "null")
  .map(() => null)
  .followedBy(whitespace);
...
const beginarray = pz
  .terminal(JsonToken.LeftBracket, "[")
  .followedBy(whitespace);
const endarray = pz
  .terminal(JsonToken.RightBracket, "]")
  .followedBy(whitespace);
const beginobject = pz
  .terminal(JsonToken.LeftBrace, "{")
  .followedBy(whitespace);
...
const member = string
  .bind((s) =>
    colon.bind(() =>
      pz.forwardRef(element).bind((e) => pz.mret([s, e] as [string, any]))
    )
  )
  .trace("member");
...
const object = members
  .bracketedBy(beginobject, endobject)
  .map((ms) => pz.initObject(ms))
  .trace("object");
/**
 ...
 */
element.target = pz
  .choose((token: pz.Token<JsonToken>) => {
    switch (token.token) {
